...
export default function LearningCircleCard({ circle, index, currentUser, onUpdate, iconConfigs }) {
  ...
  const handleJoinLeave = async () => {
    ...
    try {
      ...
      let updatedUserCircles;
      
      if (isParticipant) {
        // Leave circle
        updatedParticipants = circle.participants.filter(pId => pId !== currentUser.id);
        updatedUserCircles = (currentUser.joined_circles || []).filter(cId => cId !== circle.id);
      } else {
        // Join circle
        updatedParticipants = [...(circle.participants || []), currentUser.id];
        updatedUserCircles = [...(currentUser.joined_circles || []), circle.id];
      }
      ...
      // Update user
      await User.updateMyUserData({ joined_circles: updatedUserCircles });
      
      console.log(isParticipant ? '✓ Left circle' : '✓ Joined circle');
      
      // Trigger cache invalidation in parent
      if (onUpdate) {
        await onUpdate();
      }

    } catch (error) {
      console.error("Failed to update circle participation:", error);
    } finally {
      setIsJoining(false);
    }
  };
