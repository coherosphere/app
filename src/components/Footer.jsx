...
import { useCachedData } from '@/components/caching/useCachedData'; // New import
...
export default function Footer() {
  ...
  // Load latest health check results from aggregated data
  // NO automatic refresh - data is updated by cron job
  const { data: latestResults = [] } = useCachedData(
    ['footer', 'health-status'],
    async () => {
      // Get the 20 most recent results to ensure we capture the latest for each check_name
      // Assuming 'base44' is globally available or imported if it's a module.
      const results = await base44.entities.SystemHealthCheckResult.list('-timestamp', 20);
      return results;
    },
    'activity', // Cache category
    {
      refetchInterval: false, // DISABLED - Cron job handles updates
      refetchIntervalInBackground: false, // DISABLED
      refetchOnWindowFocus: false, // DISABLED - No need to refetch on focus
      refetchOnReconnect: false, // DISABLED - No need to refetch on reconnect
      staleTime: 5 * 60 * 1000, // Consider data stale after 5 minutes (cron runs every 5 min)
      gcTime: 10 * 60 * 1000, // Keep in cache for 10 minutes
    }
  );

  // Extract status data from latest results
  const getNetworkStatus = () => {
    // Find latest result for each check type (Mempool, Alby, Nostr)
    // We use find because results are ordered by timestamp descending, so the first match is the latest.
    const mempoolResult = latestResults.find(r => r.check_name?.includes('Mempool'));
    const albyResult = latestResults.find(r => r.check_name?.includes('Alby'));
    const nostrResult = latestResults.find(r => r.check_name?.includes('Nostr'));

    return {
      blockHeight: mempoolResult?.details?.block_height || null,
      mempoolConnected: mempoolResult ? mempoolResult.status === 'success' : null, // null if no result found
      albyConnected: albyResult ? albyResult.status === 'success' : null, // null if no result found
      nostrRelayCount: nostrResult ? (nostrResult.status === 'success' ? (nostrResult.details?.connected_relays || 0) : 0) : null, // null if no result found
      nostrTotalRelays: nostrResult ? (nostrResult.details?.total_relays || 0) : null // null if no result found
    };
  };

  const networkStatus = getNetworkStatus();

  // Calculate network icon status based on aggregated data
  const getNetworkIconStatus = () => {
    let totalConnected = 0;
    let totalServices = 0;
    let anyServiceData = false; // Flag to check if any service data was present at all

    if (networkStatus.mempoolConnected !== null) { // Only count if data was available
      totalServices += 1;
      if (networkStatus.mempoolConnected) totalConnected += 1;
      anyServiceData = true;
    }

    if (networkStatus.albyConnected !== null) { // Only count if data was available
      totalServices += 1;
      if (networkStatus.albyConnected) totalConnected += 1;
      anyServiceData = true;
    }

    if (networkStatus.nostrTotalRelays !== null) { // Only count if data was available for Nostr
      totalServices += networkStatus.nostrTotalRelays;
      totalConnected += networkStatus.nostrRelayCount;
      anyServiceData = true;
    }

    let color;
    if (!anyServiceData) {
      color = 'text-slate-400'; // No data available for any service
    } else if (totalServices === 0) {
      color = 'text-green-400'; // All services expected to be 0 (e.g., 0/0 for nostr), so all good
    } else if (totalConnected === 0) {
      color = 'text-red-400'; // Some services expected, but 0 connected
    } else if (totalConnected === totalServices) {
      color = 'text-green-400'; // All services connected
    } else {
      color = 'text-orange-400'; // Some services connected, some not
    }

    return { color, connected: totalConnected, total: totalServices };
  };

  const networkIconStatus = getNetworkIconStatus();
            ...
            <div className="h-full flex flex-col space-y-6">
              ...
              <div className="text-gray-400 mt-1 mb-1 text-sm hover:text-white transition-colors duration-200 block">
                <div className="flex items-center gap-2 mb-3">
                  <Wifi className={`w-4 h-4 ${networkIconStatus.color}`} />
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center justify-between text-gray-400">
                    ...
                    <span className="font-mono">
                      {networkStatus.blockHeight?.toLocaleString() || 'N/A'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      {networkStatus.mempoolConnected === true && <span className="text-green-400">1/1</span>}
                      {networkStatus.mempoolConnected === false && <span className="text-red-400">0/1</span>}
                      {networkStatus.mempoolConnected === null && <span className="text-slate-400">N/A</span>}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      {networkStatus.albyConnected === true && <span className="text-green-400">1/1</span>}
                      {networkStatus.albyConnected === false && <span className="text-red-400">0/1</span>}
                      {networkStatus.albyConnected === null && <span className="text-slate-400">N/A</span>}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      {networkStatus.nostrTotalRelays !== null ? (
                        <>
                          {networkStatus.nostrRelayCount > 0 && networkStatus.nostrRelayCount === networkStatus.nostrTotalRelays ? (
                            <span className="text-green-400">
                              {networkStatus.nostrRelayCount}/{networkStatus.nostrTotalRelays}
                            </span>
                          ) : networkStatus.nostrRelayCount < networkStatus.nostrTotalRelays && networkStatus.nostrRelayCount > 0 ? (
                            <span className="text-orange-400">
                              {networkStatus.nostrRelayCount}/{networkStatus.nostrTotalRelays}
                            </span>
                          ) : ( // This covers 0/X (red) and 0/0 (green, as all available are connected) cases
                            <span className={networkStatus.nostrRelayCount === networkStatus.nostrTotalRelays ? "text-green-400" : "text-red-400"}>
                              {networkStatus.nostrRelayCount}/{networkStatus.nostrTotalRelays}
                            </span>
                          )}
                        </>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span className="text-gray-400 text-sm">
                        {networkIconStatus.connected}/{networkIconStatus.total}
                      </span>
                    </Link>
                  </div>
                </motion.div>
