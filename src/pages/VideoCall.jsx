...
// Helper for ConfiguredIcon and iconConfigs as they were not defined in the original file.
// Assuming `iconName` maps directly to a Lucide icon component.
const iconMap = {
  Video: Video,
  // Add other Lucide icons here if needed, e.g., 'Home': Home, 'Settings': Settings
};

// A minimal ConfiguredIcon component that takes iconName, size, and className
// and renders the corresponding Lucide icon.
const ConfiguredIcon = ({ iconName, size, className }) => {
  const IconComponent = iconMap[iconName];
  if (!IconComponent) {
    console.warn(`ConfiguredIcon: Icon component for "${iconName}" not found.`);
    return null; // Or render a fallback icon
  }
  return <IconComponent className={`${size} ${className}`} />;
};

// A minimal iconConfigs object. Its specific content is not used by the simple ConfiguredIcon above,
// but it's kept to match the outline's usage.
const iconConfigs = {
  Video: {}, // Placeholder, actual configuration (e.g., specific color, stroke width) would go here if `ConfiguredIcon` was more complex.
};


export default function VideoCall() { // Renamed from VideoCallPage
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-3">
              <ConfiguredIcon
                iconName="Video"
                iconConfig={iconConfigs['Video']}
                size="w-12 h-12"
                className="flex-shrink-0 text-orange-500" // Added color for visual consistency
              />
              <div>
            ...
            <p className="text-lg text-slate-400 leading-relaxed">
              Join the coherosphere community for real-time video conversations.
            </p>
          </div>
          <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700 max-w-md w-full mx-auto">
            <CardContent className="p-8 text-center">
              ...
              {/* Changed h2 to text-2xl for mobile consistency as per title requirement */}
  ...
  return (
    <div className="p-4 lg:p-8 min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Adjusted height to account for the header and vertical padding */}
        <div className="h-[calc(100vh-180px)] w-full">
          <iframe
            src={iframeUrl}
            allow="camera; microphone; display-capture; autoplay"
            className="w-full h-full border-0 rounded-lg shadow-lg" // Added rounded-lg and shadow-lg for better appearance
