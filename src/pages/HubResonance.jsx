...
// Helper component for chart skeleton loading state
const ChartSkeleton = () => (
  <Card className="bg-slate-800/40 backdrop-blur-sm border-slate-700">
    <CardHeader>
      <div className="h-6 bg-slate-700 animate-pulse rounded w-48" />
    </CardHeader>
    <CardContent>
...
export default function HubResonance() {
  ...
  // Load all hubs using useCachedData
  ...
  useEffect(() => {
    ...
    const revealDelay = 50; // Small stagger between sections
    
    setTimeout(() => setSectionsReady(prev => ({ ...prev, hubHeader: true })), 0);
    setTimeout(() => setSectionsReady(prev => ({ ...prev, explainer: true })), revealDelay);
    setTimeout(() => setSectionsReady(prev => ({ ...prev, filters: true })), revealDelay * 2);
    setTimeout(() => setSectionsReady(prev => ({ ...prev, chart: true })), revealDelay * 3);
    setTimeout(() => setSectionsReady(prev => ({ ...prev, transactions: true })), revealDelay * 4);
  }, [viewingHub, isLoading]);
  ...
  // Prepare chart data with smoothing AND no-decay line
  const getChartData = () => {
    ...
    // Filter snapshots by time range and map to chart data
    let data = snapshots
      .filter(s => new Date(s.date) >= cutoffDate)
      .map(s => ({
        date: format(new Date(s.date), 'MMM dd'),
        score: s.score, // Real score WITH decay
        scoreNoDecay: s.score_no_decay || s.score, // Real score WITHOUT decay (fallback to score if not available)
        intensity: s.intensity
      }));

    // Apply smoothing ONLY to the main score (keep no-decay unsmoothed for clarity)
    if (data.length > 7) {
      const smoothed = [];
      for (let i = 0; i < data.length; i++) {
        ...
        smoothed.push({ ...data[i], score: avg }); // Keep scoreNoDecay unsmoothed
      }
      data = smoothed;
    }
  ...
  const CustomChartTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      // Find the specific lines by dataKey (more robust than index)
      const scoreData = payload.find(p => p.dataKey === 'score');
      const scoreNoDecayData = payload.find(p => p.dataKey === 'scoreNoDecay');

      const withDecay = scoreData ? scoreData.value : 0;
      const withoutDecay = scoreNoDecayData ? scoreNoDecayData.value : withDecay; // Fallback in case no-decay data is missing

      const decayLoss = withoutDecay - withDecay;
      
      return (
        <div className="bg-slate-800 border border-slate-600 rounded-lg p-3 shadow-lg">
          <p className="text-white font-semibold mb-1">{label}</p>
          <p className="text-orange-400 text-sm">
            With Decay: {withDecay.toFixed(2)}
          </p>
          <p className="text-white text-sm font-semibold">
            Without Decay: {withoutDecay.toFixed(2)}
          </p>
          <p className="text-red-400 text-xs mt-1">
            Decay Loss: -{decayLoss.toFixed(2)} points
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="p-4 lg:p-8">
        ...
        <div className="flex items-center gap-4 mb-3">
          ...
          <div>
            <h1 className="text-2xl lg:text-4xl font-bold text-white leading-tight">
              {viewingHub?.name || 'Hub Resonance'}
            </h1>
            <div className="w-16 h-1 bg-orange-500 mt-2 rounded-full"></div>
          </div>
        </div>
        <p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
          Resonance activity and impact metrics for this hub.
        </p>
      </div>
      ...
      {!sectionsReady.hubHeader ? (
        <div className="mb-8">
          <Card className="bg-slate-800/40 backdrop-blur-sm border-slate-700">
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row gap-6 items-start mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 bg-slate-700 animate-pulse rounded-full" />
                  <div>
                    <div className="h-8 bg-slate-700 animate-pulse rounded w-48 mb-2" />
                    <div className="h-4 bg-slate-700 animate-pulse rounded w-32 mb-2" />
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="text-center p-3 bg-slate-900/50 rounded-lg">
                      <div className="h-8 bg-slate-700 animate-pulse rounded w-16 mx-auto mb-2" />
                      <div className="h-4 bg-slate-700 animate-pulse rounded w-12 mx-auto" />
                    </div>
                  ))}
                </div>
              </div>
              <div className="pt-6 border-t border-slate-700 grid grid-cols-2 md:grid-cols-4 gap-4">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="text-center">
                    <div className="w-6 h-6 bg-slate-700 animate-pulse rounded mx-auto mb-2" />
                    <div className="h-6 bg-slate-700 animate-pulse rounded w-12 mx-auto mb-1" />
                    <div className="h-3 bg-slate-700 animate-pulse rounded w-16 mx-auto" />
                  </div>
                ))}
              </div>
              <div className="mt-6 pt-6 border-t border-slate-700">
                <div className="h-5 bg-slate-700 animate-pulse rounded w-48 mb-3" />
                <div className="flex flex-wrap gap-2">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="h-6 bg-slate-700 animate-pulse rounded w-24" />
                  ))}
                </div>
              </div>
            </CardContent>
      ...
      {/* Explainer Box */}
      {!sectionsReady.explainer ? (
        <div className="mb-8">
          <div className="bg-slate-800/40 backdrop-blur-sm border-slate-700 rounded-xl p-6">
            <div className="flex items-start gap-4">
              <div className="w-8 h-8 bg-slate-700 animate-pulse rounded flex-shrink-0" />
              <div className="flex-1">
                <div className="h-6 bg-slate-700 animate-pulse rounded w-64 mb-3" />
      ...
      {!sectionsReady.filters ? (
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-10 w-24 bg-slate-700 animate-pulse rounded-full" />
            ))}
          </div>
        </div>
      ) : (
                  ...
                  <UiTooltip>
                    ...
                    <TooltipContent className="bg-slate-800 border-slate-600 max-w-xs">
                      <p className="text-sm text-slate-300">
                        Shows daily snapshots of this hub's resonance score at end-of-day.
                        <strong className="text-orange-400"> Orange line:</strong> Actual score with time-decay applied (events lose value over time).
                        <strong className="text-white"> White line:</strong> Cumulative contributions without any decay.
                        The gap shows how much value has been lost to decay over time.
                      </p>
                    </TooltipContent>
                  </UiTooltip>
                </TooltipProvider>
              ...
              {chartData.length > 0 ? (
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={chartData}>
                    ...
                    <Legend />
                    {/* No-Decay line FIRST so it appears in background (but higher) */}
                    <Line
                      type="monotone"
                      dataKey="scoreNoDecay"
                      stroke="#ffffff"
                      strokeWidth={2}
                      strokeDasharray="5 5"
                      dot={false}
          ...
          <Card className="bg-slate-800/40 backdrop-blur-sm border-slate-700">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="h-10 bg-slate-700 animate-pulse rounded-full" />
                ))}
              </div>
              <div className="mb-4">
                <div className="h-10 bg-slate-700 animate-pulse rounded-full" />
              </div>
              <div className="hidden lg:grid grid-cols-9 gap-4 py-3 px-3 bg-slate-700/30 rounded-t-xl mb-0">
                {[1, 1, 2, 1, 1, 1, 1, 1, 1].map((cols, i) => (
                  <div key={i} className={`col-span-${cols} h-4 bg-slate-700 animate-pulse rounded`} />
                ))}
              </div>
              <div className="space-y-0">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="hidden lg:grid grid-cols-9 gap-4 p-3 border-b border-slate-700/50 bg-slate-800/20">
                    ...
                    <div className="col-span-1 flex justify-end">
                      <div className="h-4 bg-slate-700 animate-pulse rounded w-8" />
                    </div>
                    <div className="col-span-1">
                  ...
                  <Card key={i} className="bg-slate-700/30 border-slate-600">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1">
                          <div className="h-5 bg-slate-700 animate-pulse rounded w-3/4 mb-2" />
                          <div className="h-4 bg-slate-700 animate-pulse rounded w-1/2 mb-2" />
                          <div className="h-3 bg-slate-700 animate-pulse rounded w-1/4" />
                        </div>
                        <div className="text-right ml-4">
                          <div className="h-6 bg-slate-700 animate-pulse rounded w-16 mb-1" />
                          <div className="h-3 bg-slate-700 animate-pulse rounded w-12" />
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {[1, 2].map((j) => (
                          <div key={j} className="h-5 bg-slate-700 animate-pulse rounded w-16" />
                        ))}
                      </div>
                    </CardContent>
                  </Card>
