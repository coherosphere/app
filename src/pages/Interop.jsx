...
import { 
  ...
  Code, 
  Globe, 
  Globe2, // Added Globe2 icon for the header
  Copy, 
  ExternalLink,
  Activity,
  Filter,
...
// Define iconConfigs mapping icon names to Lucide components
const iconConfigs = {
  Globe2: {
    component: Globe2,
  },
  // Add other icons here if you plan to use ConfiguredIcon for them
};

// ConfiguredIcon component to dynamically render Lucide icons
const ConfiguredIcon = ({ iconName, iconConfig, size, className }) => {
  const IconComponent = iconConfig?.component; // Use optional chaining for safety
  if (!IconComponent) {
    console.warn(`Icon component for ${iconName} not found or invalid iconConfig provided.`);
    return null; // Or a fallback icon
  }
  return <IconComponent className={`${size} text-orange-500 ${className}`} />;
};
