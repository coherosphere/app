...
const navigationItems = [
  ...
  {
    title: "Page Overview",
    url: createPageUrl("PageAdmin"),
    icon: Layers,
    adminOnly: true
  },
  {
    title: "Icon Configuration",
    url: createPageUrl("IconAdmin"),
    icon: Palette,
    adminOnly: true
  },
  {
    title: "Stats Configuration",
    url: createPageUrl("StatsAdmin"),
    icon: Gauge, // Changed icon from ChartNoAxesGantt to Gauge
    adminOnly: true
  },
  {
  ...
  },
  {
    ...
    icon: Server, // Changed icon from Heart to Server
    adminOnly: true
  },
  {
...
function LayoutContent({ children, currentPageName }) {
  ...
  const actualPageName = React.useMemo(() => {
    ...
    // Use the last segment from the URL, or 'Dashboard' if root.
    // Fallback to currentPageName prop if it's provided and actualPageName from URL is empty.
    return pathSegments.length > 0 ? pathSegments[pathSegments.length - 1] : currentPageName || 'Dashboard';
  }, [location.pathname, currentPageName]); // currentPageName is a fallback, so it should be in dependencies too.

  // Load ALL PageInfo with caching for instant availability
  const { data: allPageInfo = [], isLoading: loadingPageInfo } = useCachedData(
    ['layout', 'allPageInfo'],
    () => base44.entities.PageInfo.list('-created_date', 500),
    'layout'
  );

  // Find PageInfo for current page
  const pageInfo = React.useMemo(() => {
    return allPageInfo.find(p => p.page_name === actualPageName) || null;
  }, [allPageInfo, actualPageName]);

  const shouldShowInitialSpinner = () => {
    // SPINNER COMPLETELY DISABLED - Pages now use Skeletons
    return false;
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      {/* CENTRALIZED INITIAL SPINNER - REMOVED, Pages use Skeletons instead */}
      ...
      <style>
        {`
              ...
              /* Hide scrollbar for mobile stats carousel */
