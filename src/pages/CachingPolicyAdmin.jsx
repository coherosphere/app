...
export default function CachingPolicyAdmin() {
  ...
  // Manage global loading indicator (for the global context)
  useEffect(() => {
  ...
  useEffect(() => {
    if (allPolicies.length > 0) {
      ...
      // This case is explicitly handled when policies are loaded and there are none.
      // In this scenario, `editedPolicy` becomes the initial draft.
      setEditedPolicy({
        id: 'new-policy',
        version: 1,
        label: 'Initial Draft',
  ...
  useEffect(() => {
    // Global Settings ready when policy loaded
    if (!isLoading && editedPolicy) {
      setSectionsReady(prev => ({ ...prev, globalSettings: true }));
    }
  }, [isLoading, editedPolicy]);

  useEffect(() => {
    // Domains Config ready when policy loaded
    if (!isLoading && editedPolicy) {
      setSectionsReady(prev => ({ ...prev, domainsConfig: true }));
    }
  }, [isLoading, editedPolicy]);

  useEffect(() => {
    // Version History ready when policies loaded
    if (!isLoading && allPolicies.length >= 0) { // allPolicies is an array, length >= 0 is always true for an array, effectively checking !isLoading
      setSectionsReady(prev => ({ ...prev, versionHistory: true }));
    }
  }, [isLoading, allPolicies]);
  ...
  const hasChanges = () => {
    ...
    // Compare only relevant fields, excluding `id` and `created_date` if necessary for comparison
    const activeCompare = JSON.parse(JSON.stringify(activePolicy));
    const editedCompare = JSON.parse(JSON.stringify(editedPolicy));
    // Optionally delete fields that shouldn't trigger a "change" status if they are backend generated
    delete activeCompare.id;
    delete editedCompare.id;
    delete activeCompare.created_date;
    delete editedCompare.created_date;
    return JSON.stringify(activeCompare) !== JSON.stringify(editedCompare);
  };
  ...
  const handleCreateNewVersion = async () => {
    ...
    try {
      ...
      // Ensure that if editedPolicy is the 'new-policy' draft, its content is used.
      // If it was null, this would be an issue, but editedPolicy is always initialized to 'new-policy' if no policies exist.
      const newPolicy = {
        ...editedPolicy, 
  ...
  const handleDiscard = () => {
    if (activePolicy) {
      ...
      // If no active policy, revert to the initial draft state (which is handled by the useEffect for allPolicies.length === 0)
        ...
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center justify-between">
            <div className="flex-1">
              <div className="h-5 w-48 bg-slate-700/30 animate-pulse rounded mb-2" />
              <div className="h-4 w-64 bg-slate-700/30 animate-pulse rounded" />
            </div>
            <div className="h-6 w-12 bg-slate-700/30 animate-pulse rounded" />
          </div>
        ))}
      </CardContent>
    </Card>
  ...
  const DomainsConfigSkeleton = () => (
    <Card className="bg-slate-800/40 backdrop-blur-sm border-slate-700 mb-8">
      <CardHeader>
        <div className="h-6 w-56 bg-slate-700/30 animate-pulse rounded" />
      </CardHeader>
      <CardContent>
          ...
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="h-6 w-32 bg-slate-700/30 animate-pulse rounded mb-2" />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                {[1, 2, 3, 4].map((j) => (
                  <div key={j}>
                    <div className="h-4 w-24 bg-slate-700/30 animate-pulse rounded mb-2" />
        ...
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="p-4 bg-slate-900/50 rounded-lg border border-slate-700 flex items-center justify-between">
              <div className="flex-1">
                <div className="h-6 w-48 bg-slate-700/30 animate-pulse rounded mb-2" />
                <div className="h-4 w-64 bg-slate-700/30 animate-pulse rounded" />
              </div>
              <div className="h-10 w-24 bg-slate-700/30 animate-pulse rounded" />
            </div>
          ))}
        </div>
      </CardContent>
  ...
  // Access denied check - show immediately without loading
  ...
  // Show "No Policy Found" only when loading is completely done AND no policy exists
  // 'editedPolicy' will have been initialized to a default draft if allPolicies is empty after loading.
  // The goal is to show this message when there are truly no *saved* policies.
  if (!isLoading && allPolicies.length === 0) {
    return (
      <div className="p-8">
        <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700">
          <CardContent className="p-8 text-center">
            <ConfiguredIcon 
              ...
              size="w-16 h-16"
              className="mx-auto mb-4"
              fallbackColor="text-orange-400/80"
            />
  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      <motion.div
        className="mb-8"
        initial={{ opacity: 1 }} // Initial state, always visible
        animate={{ opacity: 1 }} // Target state, always visible
      >
        <div className="flex items-center gap-4 mb-3">
          <ConfiguredIcon 
            iconName="DatabaseZap"
            iconConfig={iconConfigs['DatabaseZap']}
            size="w-12 h-12"
        ...
        <p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
          Configure data freshness strategies per domain for optimal performance.
        </p>
      </motion.div>
      ...
      {/* Show version badge and controls only when editedPolicy is loaded */}
      {editedPolicy && (
      ...
      {/* Global Settings - Progressive Loading */}
      {sectionsReady.globalSettings ? (
        <motion.div
              ...
              <div className="border-t border-slate-700 pt-4 mt-6">
                ...
                <div className="space-y-4">
                  ...
                  {editedPolicy?.global_settings?.freshness_overlay_enabled && (
                    <>
                      <div>
                        <Label className="text-white mb-2 block">Visible To</Label>
                        <Select
                      ...
                      {editedPolicy?.global_settings?.freshness_overlay_visible_to === 'whitelist' && (
                        <div>
                          <Label className="text-white mb-2 block">Whitelist Users</Label>
      ...
      {/* Domains Configuration - Progressive Loading */}
      {sectionsReady.domainsConfig ? (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
                  ...
                  {domainSearchQuery ? (
                    <>
                      ...
                      <Button
                        onClick={() => setDomainSearchQuery('')}
                        variant="outline"
                        className="btn-secondary-coherosphere mt-4"
                      >
                        Clear search
                      </Button>
                    </>
        ...
        <DomainsConfigSkeleton />
      )}

      {/* Version History - Progressive Loading */}
      {sectionsReady.versionHistory ? (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
