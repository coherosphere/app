...
export default function UserResonance() {
  ...
  // Progressive Loading States - START AS FALSE (zeigt sofort Skeletons)
  ...
  const isLoading = usersLoading || eventsLoading || (!viewingUser && !!targetUserId);
  useEffect(() => {
  ...
  useEffect(() => {
    // Admin banner always ready (no data dependency)
    setSectionsReady(prev => ({ ...prev, adminBanner: true }));
  }, []);

  useEffect(() => {
    // User header ready when viewingUser and resonanceScore loaded
    if (viewingUser && resonanceScore !== null) {
      setSectionsReady(prev => ({ ...prev, userHeader: true }));
    }
  }, [viewingUser, resonanceScore]);

  useEffect(() => {
    // Explainer always ready (no data dependency)
    setSectionsReady(prev => ({ ...prev, explainer: true }));
  }, []);

  useEffect(() => {
    // Time range filter always ready (no data dependency)
    setSectionsReady(prev => ({ ...prev, timeRangeFilter: true }));
  }, []);

  useEffect(() => {
    // Chart ready when snapshots loaded AND events are loaded (for no-decay projection)
    if (snapshots.length >= 0 && events.length >= 0 && !eventsLoading) {
      setSectionsReady(prev => ({ ...prev, chart: true }));
    }
  }, [snapshots, events, eventsLoading]);

  useEffect(() => {
    // Transactions list ready when events loaded
    if (!eventsLoading) {
      setSectionsReady(prev => ({ ...prev, transactionsList: true }));
    }
  }, [eventsLoading]);
  ...
  const getFilteredEvents = () => {
    ...
    switch (sortBy) {
      case 'date-desc':
        ...
        break;
      case 'date-asc':
        filtered.sort((a, b) => new Date(a.timestamp) - new Date(a.timestamp));
        break;
      case 'resonance-desc':
        filtered.sort((a, b) => (b.magnitude * (b.alignment_score || 0)) - (a.magnitude * (a.alignment_score || 0)));
        break;
      case 'resonance-asc':
        filtered.sort((a, b) => (a.magnitude * (a.alignment_score || 0)) - (b.magnitude * (a.alignment_score || 0)));
        break;
    }
  ...
  // Prepare chart data - now using REAL snapshot data for both lines
  const getChartData = () => {
  ...
  const CustomChartTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      const withDecay = payload[0]?.value || 0;
      const withoutDecay = payload[1]?.value || withDecay;
      ...
      <CardContent className="p-6">
        <div className="flex flex-col md:flex-row gap-6 items-start">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="h-7 w-48 bg-slate-700 animate-pulse rounded mb-2" />
              <div className="h-4 w-64 bg-slate-700 animate-pulse rounded" />
            </div>
          </div>
          <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="text-center p-3 bg-slate-900/50 rounded-lg">
                <div className="h-8 w-16 bg-slate-700 animate-pulse rounded mb-1 mx-auto" />
                <div className="h-3 w-12 bg-slate-700 animate-pulse rounded mx-auto" />
              </div>
            ))}
          </div>
        </div>
  ...
  const ExplainerSkeleton = () => (
    <div className="h-48 bg-slate-700/30 animate-pulse rounded-xl" />
  );

  const TimeRangeFilterSkeleton = () => (
    <div className="flex flex-wrap gap-2">
      {[...Array(6)].map((_, i) => (
        <div key={i} className="h-9 w-24 bg-slate-700 animate-pulse rounded-full" />
      ))}
    </div>
  );
    ...
    <Card className="bg-slate-800/40 backdrop-blur-sm border-slate-700">
      ...
      <CardContent>
        <div className="space-y-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-20 bg-slate-700/30 animate-pulse rounded" />
          ))}
        </div>
      </CardContent>
    </Card>
  ...
  return (
    <div className="p-4 lg:p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-4 mb-3">
          <div className="flex-shrink-0">
            {viewingUser ? (
              <img
                src={viewingUser.avatar_url || `https://api.dicebear.com/7.x/identicon/svg?seed=${viewingUser.id}`}
          ...
          <div>
            <h1 className="text-2xl lg:text-4xl font-bold text-white leading-tight">
              {viewingUser?.display_name ? `${viewingUser.display_name}'s Resonance` : 'User Resonance'}
            </h1>
            <div className="w-16 h-1 bg-orange-500 mt-2 rounded-full"></div>
          </div>
        </div>
        <p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
          {viewingUser?.display_name ? 'Resonance activity and impact metrics for this user.' : 'Your personal resonance journey — contributions, alignment, and impact over time.'}
        </p>
      </div>

      {/* Admin Banner - Progressive Loading */}
      {sectionsReady.adminBanner && isAdmin && viewingUser?.id !== currentUser?.id ? (
        <motion.div
      ...
      {/* User Header with KPIs - Progressive Loading */}
      {sectionsReady.userHeader ? (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Explainer Box - Progressive Loading */}
      ...
      {/* Time Range Filter - Progressive Loading */}
      {sectionsReady.timeRangeFilter ? (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <div className="mb-6">
          <TimeRangeFilterSkeleton />
        </div>
      )}

      {/* Chart - Progressive Loading */}
      {sectionsReady.chart ? (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
                  ...
                  <UiTooltip>
                    ...
                    <TooltipContent className="bg-slate-800 border-slate-600 max-w-xs">
                      <p className="text-sm text-slate-300">
                        ...
                        <strong className="text-orange-400"> Orange line:</strong> Your actual score with time-decay applied (events lose value over time).
                        <strong className="text-white"> White line:</strong> Your cumulative contributions without any decay.
      ...
      {sectionsReady.transactionsList ? (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
